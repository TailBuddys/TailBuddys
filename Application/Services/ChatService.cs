...
namespace TailBuddys.Application.Services
{
    public class ChatService : IChatService
    {
        ...
        //    try
        //    {
        //        if (chatToUpdate == null || message == null) return null;

        //        // Determine receiver and sender
        //        Dog? receiverDog = message.SenderDogId == chatToUpdate.SenderDogId
        //            ? chatToUpdate.ReceiverDog
        //            : chatToUpdate.SenderDog;
        //        if (receiverDog == null) return null;

        //        Dog? senderDog = chatToUpdate.SenderDogId == receiverDog.Id
        //            ? chatToUpdate.ReceiverDog
        //            : chatToUpdate.SenderDog;
        ...
        //        // 1. First immediately save and show the user's message
        ...
        //        // 2. If talking to a bot, process the response in the background
        //        if (receiverDog.IsBot == true && !senderDog.IsBot == true)
        //        {
        //            // Create a scope for the background operation
        //            using (var scope = _serviceProvider.CreateScope())
        //            {
        //                // Get fresh instances of services for the background task
        //                var backgroundChatRepository = scope.ServiceProvider.GetRequiredService<IChatRepository>();
        //                var backgroundUserRepository = scope.ServiceProvider.GetRequiredService<IUserRepository>();
        //                var backgroundOpenAiService = scope.ServiceProvider.GetRequiredService<IOpenAiService>();
        //                var backgroundTracker = scope.ServiceProvider.GetRequiredService<IDogConnectionTracker>();
        //                var backgroundNotificationService = scope.ServiceProvider.GetRequiredService<INotificationService>();
        //                var backgroundChatHubContext = scope.ServiceProvider.GetRequiredService<IHubContext<ChatHub>>();

        //                // Fire-and-forget with proper service instances
        //                _ = Task.Run(async () =>
        //                {
        //                    try
        //                    {
        //                        User? user = await backgroundUserRepository.GetUserByIdDb(receiverDog.UserId);
        //                        User? otherUser = await backgroundUserRepository.GetUserByIdDb(senderDog.UserId);

        //                        if (user != null && otherUser != null)
        //                        {
        //                            string aiResponse = await backgroundOpenAiService.GetDogChatBotReplyAsync(
        //                                user, receiverDog, otherUser, senderDog, chatToUpdate);
        ...
        //                                    ChatID = chatToUpdate.Id,
        //                                    Content = aiResponse,
        //                                    IsRead = backgroundTracker.IsDogInSpecificChat(senderDog.Id, chatToUpdate.Id)
        //                                };

        //                                await backgroundChatRepository.AddMessageToChatDb(botMessage);

        //                                bool isSenderInChat = backgroundTracker.IsDogInSpecificChat(senderDog.Id, chatToUpdate.Id);
        //                                if (isSenderInChat)
        //                                {
        //                                    await backgroundChatHubContext.Clients.Group($"Chat_{chatToUpdate.Id}")
        //                                        .SendAsync("ReceiveChatNotification", new
        //                                        {
        //                                            chatId = chatToUpdate.Id,
        ...
        //                                else
        //                                {
        //                                    var chatNotify = await backgroundNotificationService.CreateOrUpdateChatNotification(
        //                                        chatToUpdate.Id, senderDog.Id);
        //                                    await backgroundChatHubContext.Clients.Group($"DogChats_{senderDog.Id}")
        ...
        //            }
        //        }
        //        else if (!isReceiverInChat && !receiverDog.IsBot == true)
        //        {
        //            // Standard notification for non-bot receivers not in chat
        //            var chatNotify = await _notificationService.CreateOrUpdateChatNotification(
        //                chatToUpdate.Id, receiverDog.Id);
