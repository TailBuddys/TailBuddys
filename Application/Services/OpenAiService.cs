...
{
    private readonly string? _openAIApiKey = Environment.GetEnvironmentVariable("TailBuddysOpenAiKey", EnvironmentVariableTarget.User);
    ...
    public async Task<DogBreedSize> GetDogBreedFromImageUrlAsync(string publicImageUrl)
    {
        try
        {
            ...
            // Parse breed and size from OpenAI response using Regex
